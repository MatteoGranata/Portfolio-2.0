<template>
  <div class="w-full h-full font-[Roboto]" :style="{ color: textColor, backgroundColor: color }">
    <div ref="container" class="overflow-hidden w-full h-screen">
      <div ref="loader" class="w-full z-50 h-full flex justify-center items-center absolute top-0 left-0 opacity-100"
      ...
      <div class="w-full lg:h-screen relative left-0 lg:-top-5 z-0 flex justify-center mb-5 sm:mb-0">
        <video ref="video" :src="videoSrc" class="image w-full h-full lg:object-cover" autoplay muted loop playsinline
          loading="lazy">
        </video>
      </div>
